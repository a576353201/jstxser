...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="titlebg">
      ...
      <tr>
        ...
        <td>
         ...
         <span class="STYLE3">当前位置:</span>运动员管理&nbsp; &nbsp;&nbsp; &nbsp;
...
<?php
     ...
     if($_GET['name']) $sql.=" and name='$_GET[name]'";
          ...
          if(strlen($_GET['group'])){

          	$sql.=" and `group`='$_GET[group]'";
          }
          else{

          		$sql.=" and (`group`='1' or `group`=2 )";
          }
...
?>
   ...
   <form name='formSort' enctype="multipart/form-data" action='index1.php' method='get' style='min-height:50px;line-height:50px;padding-left:10px;'>

            手机号：<input type="text" name="name" value="<?php echo $_GET['name']; ?>" >

真实姓名：<input type="text" name="realname" value="<?php echo $_GET['realname']; ?>" >
...
&nbsp;&nbsp;
打法：
<select name='playkinds'>
<option value=''>不限</option>
<option value='F'  <?php if ($_GET['playkinds']=='F') echo "selected";?> >飞碟</option>
<option value='H' <?php if ($_GET['playkinds']=='H') echo "selected";?> >弧线</option>
<option value='Z' <?php if ($_GET['playkinds']=='Z') echo "selected";?> >直线</option>
</select>
&nbsp;&nbsp;
出生日期（格式:yyyy-mm-dd）：
...
年龄</span></div></td>
    <td bgcolor="#FFFFFF"><div align="center"><span class="STYLE1">
  性别</span></div></td>
                      <td bgcolor="#FFFFFF"><div align="center"><span class="STYLE1">运动员类型</span></div></td>
 ...
 <td height="20" bgcolor="#FFFFFF"><div align="center"><span class="STYLE1"><?php echo $row['realname'];?></span></div></td>
  ...
  <td height="20" bgcolor="#FFFFFF"><div align="center"><span class="STYLE1"><?php if($row['sex']==1) echo '男';else echo '女';?></span></div></td>
                  ...
                  <a href='index1.php?group=<?php echo $row['group']?>'><?php echo $user_group[$row['group']]?></a></span></div></td>
       <td height="20" bgcolor="#FFFFFF">
    <div align="center"><span class="STYLE1">
                  <?php
...
echo $update_status[$row['update']];
           ...
           if($row['update']==1){
           ?>
           <a href='update1.php?id=<?php echo $row['id']?>'  style='color:#ff0000;'>审核</a>   &nbsp; | &nbsp;
<?php }
else{
	?>
	 <a href='update1.php?id=<?php echo $row['id']?>'><?php if($row['group']==1) echo '升级';else echo "降级";?></a>   &nbsp; | &nbsp;
