...
if($agree=='1'){
    ...
    $data['update']=0;
     $data['agree']=1;
     $data['group']=4;
$data['playerid']=set_playerid(4);
add_adminlog("同意{$user['realname']}成为{$user_group[4]}");
add_msg($id,"您的[{$user_group[4]}]申请已通过");
...
$data['mark']=$_POST['mark'];
   $data['agree']=0;
    add_adminlog("拒绝{$user['realname']}成为{$user_group[4]}");
add_msg($id,"您的[{$user_group[4]}]申请已被管理员拒绝，理由：".$_POST['mark']);
}
	$db->update(tname('user'), $data, $id);
$user=get_user_byid($id);
	promptMessage('index3.php', '恭喜您！审核成功');
...
<style type="text/css">
  ...
  <form name='myform' enctype="multipart/form-data" action="update3.php?action=up&id=<?php echo $_GET['id'];?>" method="post">
...
<span style='font-size:20px;'>营业执照</span>
...
<span style='font-size:20px;'>组织机构代码证</span>
