...
if($agree=='1'){
    ...
    $data['update']=0;
     $data['agree']=1;
     $data['group']=2;
$data['playerid']=set_playerid(2);
add_adminlog("同意{$user['realname']}成为{$user_group[2]}");
add_msg($id,"您的[{$user_group[2]}]申请已通过");
...
$data['mark']=$_POST['mark'];
    ...
    add_adminlog("拒绝{$user['realname']}成为{$user_group[2]}");
add_msg($id,"您的[{$user_group[2]}]申请已被管理员拒绝，理由：".$_POST['mark']);
}
...
$user=get_user_byid($id);
	promptMessage('index1.php', '恭喜您！审核成功');
    ...
    if($user['group']==1){
    	$date['group']=2;
     $mark="您被升级为职业运动员";
     add_adminlog('用户['.$user['realname'].']升级为职业运动员');
    }else{
$date['group']=1;
   $mark="您被降级为运动员";
        add_adminlog('用户['.$user['realname'].']降级为运动员');
    }
if($_POST['mark']) $mark.=",理由：".$_POST['mark'];
add_msg($id,$mark);
$db->update(tname('user'), $date, $id);

$user=get_user_byid($id);

	$arr=array('1'=>'B','2'=>'P','3'=>'R','4'=>'O');
	if($user['playerid']){
		if(substr($user['playerid'],0,1)!=$arr[$user['group']]){

		$playerid=set_playerid($user['group']);

        $db->query("update ".tname('user')." set `playerid`='{$playerid}' where id='{$user['id']}'");
	...
	promptMessage('index1.php', '恭喜您！操作成功');
	}
exit();
}
...
<style type="text/css">
  ...
  <form name='myform' enctype="multipart/form-data" action="update1.php?action=up&id=<?php echo $_GET['id'];?>" method="post">
...
<?php echo $playkinds_arr[$player['playkinds']];?>
</div>
<?php } ?>
<?php if ($player['hand']){ ?>
<div class='line'>
...
<div class='line'  id='other_area'   style='height:120px;'>
<span  class='title'><?php echo $subinfo?>理由：</span>

  <textarea name="mark" id='mark' rows="8" cols="50" placeholder="请输入<?php echo $subinfo?>的理由" ></textarea>
...
<input class='btn01' type='submit' name='Submit' value='确认<?php echo $subinfo;?>'  onclick="return check_add();">
</div>
