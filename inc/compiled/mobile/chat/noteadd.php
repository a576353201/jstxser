...
<div class="group_note">
   ...
   <textarea id="content" placeholder="请输入要发布的通知内容"><?php echo $note['content'];?></textarea>
        ...
        <iframe src='../upload_mobile.php?fileid=file_add1&img=<?php echo $note['imgs'];?>&iframeid=upload_src1&pc=1&num=3'  id='upload_src1'  style='width:100%;height:100px;'marginwidth="0px" frameborder="0" scrolling="no" ></iframe>
    </div>
    ...
    function  click_add() {
        ...
        $.post("../api/group.php?act=<?php echo $action; ?>",{id:noteid,group_id:<?php echo $group['id']; ?>,content:$('#content').val(),imgs:$('#file_add1').val()}, function(result){
            ...
            if(res.code=='200'){
                ...
                parent.location.href='detail.php?id=<?php echo $group['id']; ?>&step=2';
            }else{
