...
<?php  for($i=0;$i<=$system['team_num1'];$i++){?>
...
<div id='lingdui_div' class='mobile_list1'  style='<?php if(count($lingdui)<1){?>display:none;<?php }?>clear:both;'>
<span  class='title22' >领队信息</span>

<div  id='lingdui' class='line' >
...
<?php  for($i=0;$i<=$system['team_num2'];$i++){?>
...
<div  class='mobile_list1' id='fulingdui_div'  style='<?php if(count($fulingdui)<1){?>display:none;<?php }?>clear:both;'>
<span  class='title22'>副领队信息：</span>

<div  id='fulingdui' class='line'>
...
<select name='jiaolian_num'  onchange="add_jiaolian1(this.value)">
<?php  for($i=0;$i<=$system['team_num3'];$i++){?>
...
<div class='mobile_list1' id='jiaolian_div'  style='<?php if(count($jiaolian)<1){?>display:none;<?php }?>clear:both;'>
<span  class='title22' >教练信息</span>


<div   class='line'  style='line-height:40px;text-align:center;'>
...
<td>
<input type="text" class="input1" style="width:50px !important;" name="jiaolian[<?php echo $index; ?>][name]" value="<?php echo $value['name']; ?>" minlength="2" maxlength="10" autofocus="" required="" autocomplete="off">
</td>
<td style='line-height:25px;'>
<input type="radio" name="jiaolian[<?php echo $index; ?>][sex]" value="1" required=""  <?php if($value['sex']==1){?>checked<?php }?>>男<br>
 <input type="radio" name="jiaolian[<?php echo $index; ?>][sex]" value="2" required="" <?php if($value['sex']==2){?>checked<?php }?>>女
</td>

<td style='line-height:25px;'>
<input type="radio" name="jiaolian[<?php echo $index; ?>][waiji]" value="1" required="" <?php if($value['waiji']==1){?>checked<?php }?>>是<br>
 <input type="radio" name="jiaolian[<?php echo $index; ?>][waiji]" value="2" required="" <?php if($value['waiji']==2){?>checked<?php }?>>否
</td>
<td style='position: relative;'>
  <input type="text" style="width:70px;" placeholder="上年度锦标赛代表单位" name="jiaolian[<?php echo $index; ?>][danwei]" value="<?php echo $value['danwei']; ?>" minlength="2" maxlength="10" autofocus="" autocomplete="off"

  id='jiaolian_danwei1_<?php echo $index; ?>'   onclick="set_danwei(this.value,<?php echo $index; ?>,'jiaolian',0);" oninput="set_danwei(this.value,<?php echo $index; ?>,'jiaolian',1);"
  >
...
<div class='line'  style='clear:both;line-height:40px;'>
<span  class='title' style='float:left;'> <span class='must'>*</span>搜索运动员：</span>
<div   style='float:left;'>
<input type='text' id='playerid' class='input1' value='' placeholder="请输入编号">
<input type='text' id='realname' class='input1' value='' placeholder="请输入姓名">
<select id='playersex'>
