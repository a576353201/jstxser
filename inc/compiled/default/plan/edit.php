...
<ul class="nav1" style="position: fixed;left: 0px;width: 100%;top:0px;z-index: 10">
    <li class="">追号模式</li>
    <li class="" >智能模式</li>
...
<ul class="addline" style="margin-top: 50px;">
    <li>选择彩种：</li>
    <li>
        <select id="gamekey" onchange="change_game(this.value);" disabled="disabled">
        ...
        <span id="showtime">(距下期还剩：<span id="open_time" style="color: #2319dc;">00:00:00</span>)</span>
        <span id="period_tips"></span>
    </li>
</ul>
...
<ul class="addline">
    <li>选择玩法：</li>
    <li>
        <select id="wanfa" onchange="change_wf(this.value);" >
        ...
        <select id="wanfa1" style="display: none" onchange="set_playhtml(wf1,this.value);">
        ...
        &nbsp;
        <input type="checkbox" name="number_trend" value="hot" onclick="return click_trend(1);">冷热
...
<script>
    ...
    var game_type='<?php echo $game['type']; ?>';
    ...
    var gamelist=JSON.parse('<?php echo json_encode($gamelist); ?>');
    listen_period();
    var wanfa_arr=JSON.parse('<?php echo json_encode($wanfa_arr); ?>');
    ...
    var vip=parseInt('<?php echo $user['vip']; ?>');
    var  wanfa_vip=['z12','z24','hz','kd','rxfs'];
    change_game('<?php echo $game['showkey']; ?>');
    change_tab(method);
    change_wf(wf1);
