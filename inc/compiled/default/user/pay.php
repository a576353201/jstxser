...
<script>
    var paytype='<?php echo $_GET['type']; ?>';
    var payinfo=JSON.parse('<?php echo $payinfo; ?>');
   ...
   var min=1;
    ...
    $(".real-ipt").on("input", function () {
        ...
        if (/^[0-9]*$/g.test($(this).val())) {  //有值且只能是数字（正则）
            ...
            if (len === 6) {
                ...
                var postdata={type:paytype,money:document.getElementById('money').value,pwd:pwd,id:<?php echo $_GET['id']; ?>,payinfo:JSON.stringify(payinfo)};

                $.post("../api/pay.php?act=paymoney",postdata, function(result){
                    layer.close(loading);
                    ...
                    if(result.code==200){
                        ...
                        if(paytype=='redpacket'){
                            ...
                            console.log(result);
                            sendpacket(data,result.message);
                        ...
                        var index=  layer.confirm('您的账户余额不足', {
                            ...
                            parent.click_recharge(document.getElementById('money').value);
                            var index = parent.layer.getFrameIndex(window.name);
                            parent.layer.close(index);
                        }, function(){
    ...
    <?php if($type=='redpacket'){?>
    function  sendpacket(data,msg) {
        if(data>0){
         var isgroup=<?php echo $isgroup; ?>;
            var mid ='m' +Math.random().toString(36).substring(2);
            var sendData={title:'<?php echo $_GET['title']; ?>',status:1,id:data,isgroup:isgroup,type:<?php echo $_GET['redtype']; ?>,isopen:0};
            if(isgroup==1){
                var data = {
                    userid: '<?php echo $_SESSION['userid']; ?>',
                    group_id:<?php echo $_GET['chatid']; ?>,
                    type: 'group',
                    msgtype: 'redpacket',
                    content: sendData,
                    mid: mid
                ...
                var data = {
                    userid: '<?php echo $_SESSION['userid']; ?>',
                    friend_uid: <?php echo $_GET['chatid']; ?>,
                    type: 'chat',
                    msgtype: 'redpacket',
                    content: sendData,
                    mid: mid
            ...
            parent.layer.msg( msg,{ type: 1, anim: 2 ,time:1000});
