...
if($_FILES['imgurl']['name'] != ''  )
{
 ...
 $path1=str_replace("../", "", $path);
  ...
  echo "parent.document.myform.".$returnid.".value='".$path1.$filename."';\n";
   if($_POST['pre']==1) {
  ...
  echo "alert('上传成功,图片大小".$filesize."K');location.href='upload.php?returnid=$returnid&path=$_POST[path]&image=1&pre=$_POST[pre]&mark=$_POST[mark]";
 ...
 $path1=str_replace("../", "", $path);
  ...
  echo "alert('上传成功,附件大小".$filesize."K');location.href='upload.php?returnid=$returnid&path=$_POST[path]&pre=$_POST[pre]&mark=$_POST[mark]";
